"""Run Qwen zero-shot inference for one config."""
...
def main() -> None:
    ...
    model_family = str(config.get("model", {}).get("family", "qwen")).strip().lower()
    context_type = config.get("context", {}).get("type", "sentence")
    use_rag = bool(config.get("rag", {}).get("enabled", False))
    model_name = config.get("model", {}).get("name", "Qwen/Qwen2.5-72B-Instruct")

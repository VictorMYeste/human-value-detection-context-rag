"""Run Mistral zero-shot inference for one config."""
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run Mistral inference.")
...
def main() -> None:
    ...
    model_family = str(config.get("model", {}).get("family", "mistral")).strip().lower()
    ...
    LOGGER.info("=" * 80)
    LOGGER.info(
        "Run: model=%s variant=%s context=%s rag=%s split=%s eval=%s dry_run=%s",
        model_family,
        model_name,
        context_type,
        use_rag,
    ...
    LOGGER.info("Running %s with config %s on split %s", model_family, args.config, args.split)
    ...
    if args.eval:
        ...
        pred_path = (
            Path(results_dir)
            / "predictions"
            / f"{model_family}_{context_type}_{rag_suffix}_{model_slug}_{args.split}.jsonl"
        )
